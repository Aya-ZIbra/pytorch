...
StreamId get_external_stream_id(sycl::queue ext_queue, DeviceIndex device_index)
    const {
  ...
  // from the unique_ptr.
  auto queue_ptr = std::make_unique<sycl::queue>(ext_queue);
  stream_id = reinterpret_cast<StreamId>(queue_ptr.get());
  TORCH_CHECK(
      !(stream_id & 1),
      "StreamId of external SYCL queue is not properly aligned");
  device_external_stream.emplace(ext_queue, std::move(queue_ptr));
  return stream_id;
}
...
XPUStream getStreamFromExternal(
  ...
  initXPUStreamsOnce();
  check_device_index(device_index);
  auto stream_id = get_external_stream_id(ext_queue, device_index);
